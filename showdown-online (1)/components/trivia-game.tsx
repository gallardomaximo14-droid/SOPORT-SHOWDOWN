  ...
  const createRoom = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setPlayersReady({ [data.playerId]: false })
      } else {
  ...
  const joinRoom = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Convertir jugadores de la API al formato local
        const convertedPlayers: Player[] = data.room.players.map((p: any) => ({
          id: p.id,
          name: p.name,
          score: p.score,
          correctAnswers: 0,
        ...
        const readyState: { [key: string]: boolean } = {}
        data.room.players.forEach((p: any) => {
          readyState[p.id] = p.ready
        })
            ...
            <div className="space-y-6">
              ...
              {gameMode === "join" && (
                <>
                  ...
                  <button
                    onClick={joinRoom}
                    disabled={!playerName.trim() || !joinRoomCode.trim() || loading}
